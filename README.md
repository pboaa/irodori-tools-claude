...
- **絵文字スタイル制御**: 公式 [`EMOJI_ANNOTATIONS.md`](https://huggingface.co/Aratako/Irodori-TTS-500M/blob/main/EMOJI_ANNOTATIONS.md)
  の全絵文字を効果ラベル付きで一覧表示。クリックで選択リストに追加（チップ表示・削除可）し、
  各生成で選択分からランダムに採用。付与方法は **ランダムにN個追加（優先）/ 文頭 / 文末 /
  文頭＋文末 / なし**。ランダム個数は **固定 or 範囲（1〜10）** を選択可能。
  採用された絵文字は JSON に記録。
...
設定は上部2カラム、スクリプトプレビューは最下部のアコーディオン（デフォルト折りたたみ。
コピー/ダウンロードはヘッダから直接実行可）。ランダム生成を主目的とするため、
`seed` と主要な CFG スケールは既定で「範囲」が有効になっています。
