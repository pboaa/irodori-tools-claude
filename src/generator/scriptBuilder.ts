...
export function buildPs1(config: GenConfig): string {
  ...
  const seedRepeat = Math.max(1, Math.floor(config.seedRepeat));
  const seedActive = active.some((p) => p.flag === 'seed');
  ...
  L.push('$Index = 0');
  if (seedActive && seedRepeat > 1) L.push('$SeedHold = 0');
  L.push('');
  ...
  // Reuse one seed for N consecutive generations when requested.
  if (seedActive && seedRepeat > 1) {
    L.push(`    if ((($Index - 1) % ${seedRepeat}) -ne 0) { $Seed = $SeedHold } else { $SeedHold = $Seed }`);
  }
...
export function buildBat(config: GenConfig): string {
  ...
  if (seedActive && seedRepeat > 1) L.push('set /a SEEDHOLD=0');
  L.push('');
  ...
  // Reuse one seed for N consecutive generations when requested.
  if (seedActive && seedRepeat > 1) {
    L.push(`      set /a SEEDMOD=(!INDEX!-1) %% ${seedRepeat}`);
    L.push('      if !SEEDMOD! NEQ 0 (set /a Seed=SEEDHOLD) else (set /a SEEDHOLD=Seed)');
  }
...
export function buildPy(config: GenConfig): string {
  ...
  L.push(`SEED_REPEAT = ${Math.max(1, Math.floor(config.seedRepeat))}  # reuse one seed for N consecutive gens`);
  ...
  L.push('            "command": "generate_tts.py (load-once driver)", "rating": 0,');
  L.push('        }');
