...
export function defaultParams(): ParamRange[] {
  return [
    ...
    { flag: 'num-steps', label: 'num-steps', type: 'int', kind: 'off', fixed: 40, min: 20, max: 60, decimals: 0, slider: true, sliderMin: 1, sliderMax: 100, step: 1, default: 40, desc: '拡散ステップ数。多いほど高品質だが遅い。少なすぎると音が荒れる（既定40）。' },
    { flag: 'cfg-scale-text', label: 'cfg-scale-text', type: 'float', kind: 'range', fixed: 3.0, min: 2.0, max: 4.0, decimals: 2, slider: true, sliderMin: 0, sliderMax: 10, step: 0.1, default: 3.0, desc: 'テキストへの忠実度。高いほど発話内容に正確だが硬くなりがち、低いと自由・不安定（既定3.0）。' },
    ...
    { flag: 'cfg-scale-speaker', label: 'cfg-scale-speaker', type: 'float', kind: 'range', fixed: 5.0, min: 4.0, max: 6.0, decimals: 2, slider: true, sliderMin: 0, sliderMax: 10, step: 0.1, default: 5.0, desc: '参照話者への忠実度。高いほど声が似るが不自然になりやすい、低いと崩れる（既定5.0）。' },
    { flag: 'duration-scale', label: 'duration-scale', type: 'float', kind: 'off', fixed: 1.0, min: 0.9, max: 1.1, decimals: 2, slider: true, sliderMin: 0.5, sliderMax: 2.0, step: 0.05, default: 1.0, desc: '生成される音声長の倍率。>1 でゆっくり長く、<1 で速く短く（既定1.0）。' },
...
export function defaultConfig(): GenConfig {
  return {
    ...
    refWav: '',
    emojiEntries: [
      // A mild example: sometimes append one 😊 at the end.
